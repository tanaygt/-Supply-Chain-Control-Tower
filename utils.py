# utils.py — shared helpers + design system
...
DATA_FILE = "data.json"
DEFAULT_DATA = {"inventory": [], "orders": [], "shipments": [], "vendors": [], "demand_history": {}}
...
[data-testid="stMetricLabel"] {{ color: {t["text_muted"]} !important; font-size: 12px !important; font-weight: 600 !important; text-transform: uppercase !important; letter-spacing: 0.05em !important; }}
[data-testid="stMetricValue"] {{ color: {t["text"]} !important; font-family: 'JetBrains Mono', monospace !important; font-size: 28px !important; }}
...
.kpi-label {{
    ...
    color: {t["text_muted"]}; font-weight: 700; margin-bottom: 6px;
}}
.kpi-value {{
    font-family: 'JetBrains Mono', monospace;
    font-size: 36px; font-weight: 700; color: {t["text"]}; line-height: 1;
}}
...
.page-title {{
    font-size: 24px; font-weight: 800; color: {t["text"]};
...
.pipeline-col {{
    ...
    box-shadow: {t["shadow"]}; min-height: 120px;
}}
.pipeline-col-title {{
    font-size: 10.5px; text-transform: uppercase; letter-spacing: 0.1em;
    color: {t["text_muted"]}; font-weight: 700; margin-bottom: 12px;
...
.footer {{
    ...
    font-size: 11.5px; color: {t["text_muted"]};
    border-top: 1px solid {t["border_soft"]}; margin-top: 48px;
...
.sidebar-stat {{
    ...
    padding: 6px 0; font-size: 12px; color: {t["text_muted"]}; font-weight: 500;
}}
...
def section_header(label: str):
    st.markdown(f'<div class="section-header"><span class="sh-dot"></span>{label}</div>',
                unsafe_allow_html=True)
...
def kpi_cards_css():
    return ""  # kept for backward compat; CSS now in inject_css()
